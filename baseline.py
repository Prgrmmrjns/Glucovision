...
dietitian_features = ['insulin', 'calories']
...
def optimize_for_patient(patient, prediction_horizon, base_control_points, approach='dietitian'):
    ...
    optimization_features = dietitian_features if approach == 'dietitian' else features
    ...
    study = optuna.create_study(study_name=f"{patient}_{approach}", direction="minimize", sampler=optuna.samplers.TPESampler(seed=random_seed), storage=f"sqlite:///optuna_{patient}_{approach}.db", load_if_exists=True)
    ...
    def objective(trial):
        ...
        df = add_features(params, optimization_features, (glucose_data_copy, combined_data_copy), prediction_horizon)

        # --- Use Pre-calculated Normalized Target --- 
        feature_impacts = df[optimization_features].copy()
    ...
    print(f"Completed optimization for patient {patient} with approach {approach}, best score (1 - abs(corr)): {study.best_value:.4f}")
    os.remove(f"optuna_{patient}_{approach}.db")
        
    return f"{patient}_{approach}", best_params
...
base_control_points = {
    ...
    'calories': [[0.2, 1.0], [1.0, 2.5], [2.5, 10.0]],        # Medium rise, peak 1-2.5h, return 3-6h
}
...
if load_params:
    with open('parameters/patient_bezier_params_baseline.json', 'r') as f:
        patient_params = json.load(f)
else:
    # Optimize for dietitian approach
    dietitian_results = Parallel(n_jobs=min(len(patients), n_jobs))(
        delayed(optimize_for_patient)(patient, 6, base_control_points, 'dietitian') 
        for patient in patients
    )
    
    # Store results
    patient_params = {}
    for result in dietitian_results:
        key, value = result
        patient_name, approach = key.split('_', 1)
        if patient_name not in patient_params:
            patient_params[patient_name] = {}
        patient_params[patient_name][approach] = value
    
    # Save as JSON
    with open('parameters/patient_bezier_params_baseline.json', 'w') as f:
        json.dump(patient_params, f, indent=2)

for prediction_horizon in prediction_horizons:
    # Process per patient for evaluation
    for patient in patients:
        # Get patient-specific feature parameters based on approach
        if 'dietitian' in patient_params[patient]:
            bezier_params = patient_params[patient]['dietitian']['bezier_points']
        else:
            # Fallback if specific approach params not found
            bezier_params = patient_params[patient]['bezier_points']
        
        # Process data with dietitian features
        data = add_features(
            {k: v for k, v in bezier_params.items() if k in dietitian_features}, 
            dietitian_features, 
            get_data(patient, prediction_horizon), 
            prediction_horizon
        )
...
df.to_csv('results/evaluation_metrics_baseline.csv', index=False) 